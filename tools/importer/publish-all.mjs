...
// Full-site publisher: for EVERY local content page (content/**.plain.html),
// upload the source to Document Authoring at its site path, preview it, then
// publish (live). Force-all: does not skip already-published pages, but is
// resumable via a done log so an interrupted run continues where it stopped.
//
// Mirrors the news publisher's transport: curl (credential injection),
// bounded concurrency, exponential backoff on HTTP 429.
//
// The site path is the .plain.html path relative to content/, minus the
// extension. DA source uploads use "<path>.html"; admin preview/live use the
// extensionless path. Wrappers (body/main) match the news publisher so the DA
// document structure is identical.
//
// Usage: node tools/importer/publish-all.mjs
//   [--concurrency=N] [--limit=N] [--offset=N] [--no-upload]
//   [--reset]   start a fresh done log (ignore prior progress)
...
const DONE = path.join(PROG, 'publish-all-done.txt');
const FAIL = path.join(PROG, 'publish-all-failed.tsv');
...
// Collect every .plain.html under content/ as a site-relative path (no ext).
const rels = [];
const walk = (d) => {
...
async function run(rel) {
  ...
  if (!noUpload) {
    const tmp = path.join(os.tmpdir(), `pubany-${rel.replace(/\W/g, '_')}.html`);
